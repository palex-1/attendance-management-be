...
public class MultipleSheetXLSDocumentBuilder implements MultipleTableToDocumentBuilder {

	private static final Logger LOGGER = LoggerFactory.getLogger(MultipleSheetXLSDocumentBuilder.class);
	...
	private void buildCellValue(HSSFWorkbook workbook, Row row, CellValue value, int colNumber) {
	...
	private void decorateLabelCell(HSSFWorkbook workbook, ColumnLabel label, Cell cell) {
	...
	private Row buildRow(int rowIndex, HSSFSheet sheet) {
	 ...
	 * @throws IOException
	 */
	private InputStream buildInputStreamFile(HSSFWorkbook workbook) throws IOException {
	...
	private void buildSingleTable(Table tabella, HSSFWorkbook workbook){
		HSSFSheet sheet = workbook.createSheet(tabella.getTableName());
